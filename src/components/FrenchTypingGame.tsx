...
const FrenchTypingGame: React.FC = () => {
  ...
  const speakLine = useCallback((text: string) => {
    ...
    const lang = LANG_VOICES[langIdx] || "en-US";
    utter.lang = lang;
    utter.rate = 0.9;
    // only use female voices — preferred picks, then fallback, skip if none
    const voices = window.speechSynthesis.getVoices();
    const langPrefix = lang.split("-")[0];
    const preferred: Record<string, RegExp> = {
      en: /zoe|samantha|karen/i,
      fr: /am[eé]lie|shelley.*fr|sandy.*fr/i,
      zh: /tingting|meijia|sinji/i,
    };
    const female = voices.find(
      (v) => v.lang.startsWith(langPrefix) && preferred[langPrefix]?.test(v.name)
    ) || voices.find(
      (v) => v.lang.startsWith(langPrefix) && !/male|thomas|daniel|jacques|albert|fred/i.test(v.name)
    );
    if (!female) return;
    utter.voice = female;
